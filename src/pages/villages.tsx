  ...
  useEffect(() => {
    const loadAllVillages = async () => {
      ...
      try {
        ...
        const uniqueVillages = Array.from(villagesMap.entries()).map(
          ([key, value]) => ({
            key,
            name: value.name,
          ...
          <VirtualizedList
            ...
            renderItem={(village) => (
              <EntityItem
                ...
                description={
                  <>
                    <strong className="text-black">{village.name}</strong>, a
                    village in{" "}
                    <strong className="text-black">{village.mandalName}</strong>{" "}
                    mandal,{" "}
                    <strong className="text-black">
                      {village.districtName}
                    </strong>{" "}
                    district,{" "}
                    <strong className="text-black">{village.stateName}</strong>,
                    India. The village code is{" "}
